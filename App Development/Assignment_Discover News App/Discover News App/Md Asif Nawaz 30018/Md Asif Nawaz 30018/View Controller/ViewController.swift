...
class ViewController: UIViewController {
    ...
    var initialRefresh : [String: Date] = ["All News" : Date(),
                                       "Health":Date(),
                                       "Science":Date(),
                                       "Sports":Date(),
                                       "Business":Date(),
                                       ...
                                       "Entertainment":Date(),
                                       "General":Date()]
//    var isAlreadyCached : Bool =  false
    ...
    func loadCachedOrNewData(category : String? = nil){
        ...
        case .success(let articleModelList):
            ...
            if articleModelList.count == 0 || NetworkManager.shared.isPaginating == true {
                ...
                //defaults.set(Date(), forKey: "lastRefreshed")
                initialRefresh.updateValue(Date(), forKey: currentCategory!)
                defaults.set(initialRefresh, forKey: "lastRefreshed")
                print(initialRefresh)
